...
class PlayerStatsList extends React.Component {
  ...
  handleSort = e => {
    ...
    const newAsc = sortedBy === sortBy ? !asc : sortBy === 'lastName';
    ...
    const filteredPlayers = this.state.skaters
      ...
      : newSorting.filter(p => p.primaryPosition.code === 'G' && p.stats[0].stat.shotsAgainst);
  ...
  togglePlayers = e => {
    ...
    if (!skaters) {
      ...
      const players = newSorting.filter(p => p.primaryPosition.code === 'G' && p.stats[0].stat.shotsAgainst);
      const pages = paginatePlayers(players, this.state.perPage);
      this.setState({
        players: newSorting,
        skaters,
  ...
  render() {
    ...
    return (
      <div className="container">
        ...
        {pages.length > 1 && <PageController togglePages={this.togglePages} currentPage={this.state.currentPage} />}
        {pages.length > 0 &&
          createPlayerList(this.state.pages[this.state.currentPage], skaters, this.handleSort, setPlayer)}
        {pages.length > 1 && <PageController togglePages={this.togglePages} currentPage={this.state.currentPage} />}
      </div>
    );
  }
